...
this.addRangeCircle = function() {
    var h = 1;
    var size = 50; // Half the width/height of the 9x9 grid (total range will be 100x100 units)
    var body = [];

    // Vertical lines (9 lines, spaced 12.5 units apart from -50 to 50)
    for (var x = -size; x <= size; x += size / 4) {
        body.push(x, -size, h, x, size, h); // Vertical line from -50 to 50
    }

    // Horizontal lines (9 lines, spaced 12.5 units apart from -50 to 50)
    for (var y = -size; y <= size; y += size / 4) {
        body.push(-size, y, h, size, y, h); // Horizontal line from -50 to 50
    }
    ...
    this.handleContextMenuEvent = function(event){
        ...
        case "cm-change-id-to-ref-in-scene":
            ...
            //跨帧/跨场景同步对象 ID”的操作，用于视频或时序标注工具中。
    ...
    this.on_load_world_finished= function(world){
        ...
        document.title = "Auto 3D annotation Pro -" + world.frameInfo.scene;
