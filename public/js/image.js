...
function BoxImageContext(ui){
    ...
    function draw_box_on_image(ctx, box, box_corners, trans_ratio, selected){
        ...
        if (!selected){
            ...
            ctx.strokeStyle="#FF00FF";  //### #ff00ff
            ctx.fillStyle="rgba(255,255,0,0.3)"; //yellow, old: 255,0,255, 0.2
        }
...
//    }
    render_2d_image() {
        ...
        var svg = this.ui.querySelector("#maincanvas-svg");
        var img = this.ui.querySelector("#svg-image");

        if (img && svg) {
            // Remove existing border to avoid duplicates
            var existingBorder = svg.querySelector("rect.border-rect");
            if (existingBorder) svg.removeChild(existingBorder);
            var existingInner = svg.querySelector("rect.inner-rect");
            if (existingInner) svg.removeChild(existingInner);

            // ==== 主边框：纯色 ====
            var mainRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
            mainRect.setAttribute("x", "0");
            mainRect.setAttribute("y", "0");
            mainRect.setAttribute("width", img.getAttribute("width"));
            mainRect.setAttribute("height", img.getAttribute("height"));
            mainRect.setAttribute("rx", "40"); // 圆角半径
            mainRect.setAttribute("ry", "40");
            mainRect.setAttribute("fill", "none");
            mainRect.setAttribute("stroke", "#ffff33"); // 边框颜色
            mainRect.setAttribute("stroke-width", "10"); // 边框宽度
            mainRect.classList.add("border-rect");

            // 插入顺序：先 main，最后 image 在最底层
            var imageElement = svg.querySelector("#svg-image");
            if (imageElement) {
                svg.insertBefore(mainRect, imageElement.nextSibling);
            } else {
                svg.appendChild(mainRect);
            }

            console.log("Simple border with rounded corners added, size:", img.getAttribute("width"), img.getAttribute("height"));
        }
    }
...
//    console.log('Roy: bounding box is:', points3d_homo_to_image2d(box3d, calib));
