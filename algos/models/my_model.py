...
class model_configs():
    def __init__(self, num_class =3):
        ...
        self.head_conv = 64
        self.num_classes = 3 ### 3 for kittie, 16 for nuesenes
        self.num_center_offset = 2
        self.num_z = 1
        self.num_dim = 3
        self.num_direction = 2  # sin, cos
        self.num_layers = 18
...
class PoseResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            if (head == 'hm_cen' or head == 'cen_offset'):
                ret[head] = torch.clamp(self.sigmoid(final_out), min=1e-4, max=1 - 1e-4) # clamp: to avoid gradient vanishing (loss nan)
            else:
                ret[head] = final_out  #torch.clamp(x.sigmoid_(), min=1e-4, max=1 - 1e-4)
...
def get_net():
    block_class, layers = resnet_spec[configs.num_layers]
    model = PoseResNet(block_class, layers, heads, head_conv=head_conv)  ## head['hm_cen']   =3 or 16
    model.init_weights(configs.num_layers, pretrained = configs.is_pretrained)
    return model