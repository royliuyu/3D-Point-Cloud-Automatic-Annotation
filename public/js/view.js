...
function ViewManager(mainViewContainer, webglScene, webglMainScene, renderer, globalRenderFunc, on_box_changed, cfg){
    ...
    function create_main_view(scene, renderer, globalRenderFunc, container, on_box_changed){
        ...
        view.backgroundColor=　(document.documentElement.className == "theme-dark") ? new THREE.Color( 0.15, 0.15, 0.15 ) : new THREE.Color( 0.6, 0.6, 0.6 );
        view.zoom_ratio = 10.0; //useless for mainview ###Roy

        let camera = new THREE.PerspectiveCamera( 65, container.clientWidth / container.clientHeight, 1, 500 );
        camera.position.x = 0;
        camera.position.z = 50;
        camera.position.y = 0;
        camera.position.set(-15, 0, 10); // Roy setup
        camera.up.set( 0, 0, 1);
        camera.lookAt( 0, 0, 0 );
...
function BoxView(ui, mainViewContainer, scene, renderer, viewManager){
    ...
    function createTopView(scene, renderer, container){
        ...
        view.backgroundColor = 　(document.documentElement.className == "theme-dark") ? new THREE.Color( 0.12, 0.12, 0.12 ) : new THREE.Color( 0.95, 0.9, 0.9 );
        view.container = container;
        view.scene = scene;
        view.renderer = renderer;
    ...
    function createSideView(scene, renderer, container){
        ...
        view.backgroundColor=(document.documentElement.className == "theme-dark") ? new THREE.Color( 0.12, 0.12, 0.12 ) : new THREE.Color( 0.9, 0.95, 0.9 );
        view.container = container;
        view.scene = scene;
        view.renderer = renderer;
    ...
    function createBackView(scene, renderer, container){
        ...
        view.backgroundColor= (document.documentElement.className == "theme-dark") ? new THREE.Color( 0.12, 0.12, 0.12 ) : new THREE.Color( 0.9, 0.9, 0.95 );
        view.container = container;
        view.scene = scene;
        view.renderer = renderer;
